...
want+ possible. Beware that this configuration uses a cutting-edge
version of Emacs, so YMMV.
...
For pretty much every programming language it's interesting to have
automatic pair close insert.
...
(use-package evil
  ...
  :config
  (evil-mode +1)
  (custom-set-variables '(evil-want-Y-yank-to-eol t t)))
#+end_src
...
(use-package evil-collection
  ...
  :config
  (setq evil-collection-mode-list (cl-set-difference evil-collection-mode-list '(dashboard ibuffer lispy)))
...
=corfu.el= (Complete Overlay Region FUnction) is a pretty way to get
in-buffer completion and narrowing within a popup. Issue is, it doesn't
play along well with LSP servers in general (at least with both =lsp-mode=
and =eglot=). Really hope that eventually changes in the future, 'cause I
like Corfu more than I like =company=.
...
Attempt #37483745 to make in-buffer completion look pretty. That is,
when =eglot= is turned on. Wish I wouldn't need to use =company=, but LSP
completion is wonky with simpler completion systems. That is, =company=
...
Adds information on some selection functions such as =find-file= and
others. Feels good to use Emacs built-in function in a prettier way.
...
While I've somewhat used =neotree.el=, I believe that =treemacs= is turning
out to be a better option, as it offers a bunch of extra integrating
packages and is overall more popular than the former.
...
To ensure that Emacs uses the same path and environment as =shell= uses, I
use =exec-path-from-shell=. That way commands that work on the =shell= will
...
Non-bloated =lsp-mode= alternative. Always strive for leaner, simpler
alternatives (and try to contribute to them when possible).
...
(use-package eglot
  ...
  (:keymaps 'hcps/leader-map
   ...
   "m e f" #'eglot-format
   "m e r" #'eglot-rename
   "m e a" #'eglot-code-actions
   "m e C-r" #'eglot-reconnect
   "m e C-s" #'eglot-shutdown
   "m e C-a" #'eglot-shutdown-all)
  :custom
...
Supposedly this works better than =smartparens= when using Evil
keybindings. Hopefully this is the case, 'cause I don't really use
=smartparens= keybindings the way (nor the frequency) I should be.
...
(use-package org
  ...
  (dolist (num (number-sequence 1 8))
    (custom-set-faces `(,(intern (format "org-level-%d" num)) ((t (:family "Iosevka Etoile" :weight regular)))))))
#+end_src
...
(use-package ox
  ...
  :straight htmlize
  :preface
  (defconst org-export-default-output-folder "org-exports"
...
Lisp function does not specify a special indentation."
  ...
  :hook
  ((lisp-mode-hook . lispyville-mode)
...
(use-package markdown-mode
  ...
  :config
  (custom-set-faces '(markdown-header-face ((t (:family "Iosevka Etoile" :weight regular :height 1.0)))))
  (dolist (num (number-sequence 1 6))
    (custom-set-faces `(,(intern (format "markdown-header-face-%d" num)) ((t (:family "Iosevka Etoile" :weight regular :height 1.0)))))))
#+end_src
