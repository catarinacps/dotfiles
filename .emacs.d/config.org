...
- [[#personal][Personal]]
  - [[#functions][Functions]]
    ...
    - [[#crux-facilitators][Crux facilitators]]
...
(use-package aggressive-indent
  ...
  ((emacs-lisp-mode . aggressive-indent-mode)
   ...
   (c-mode-common . aggressive-indent-mode))
  :custom
...
(use-package ox
  ...
  (defun org-export-add-backends (new-backends)
    "Adds a new backend to the backends list."
    (progn
      (setq org-export-registered-backends
            (cl-remove-if-not
             (lambda (backend)
               (let ((name (org-export-backend-name backend)))
                 (or (memq name new-backends)
                     (catch 'parentp
                       (dolist (b new-backends)
                         (and (org-export-derived-backend-p b name)
                              (throw 'parentp t)))))))
             org-export-registered-backends))
      (let ((new-list (mapcar #'org-export-backend-name
                              org-export-registered-backends)))
        (dolist (backend new-backends)
          (cond
...
(use-package clang-format
  ...
  (defun cc-format-on-save-hook ()
    (when c-buffer-is-cc-mode
      (clang-format-buffer)))
  (defun hcps/clang-format-region-or-buffer ()
    (interactive)
  ...
  (:states '(normal visual)
   ...
   :prefix hcps/leader-key
   "i" 'hcps/clang-format-region-or-buffer)
  :custom
...
(defun elisp-coding-hook ()
  (lisp-coding-hook)
  (recompile-elc-on-save))

(add-hook 'emacs-lisp-mode-hook #'elisp-coding-hook)
#+end_src
...
(use-package auctex
  ...
  ((latex-mode . turn-on-reftex)
   ...
   (latex-mode . rainbow-delimiters-mode-enable)
...
(add-to-list 'auto-mode-alist '("bspwmrc\\'" . bspwm-config-mode))
...
#+begin_src emacs-lisp :tangle yes
(defun hcps/indent-region-or-buffer ()
  "Uses crux to make it work with the whole buffer when there isn't a region enabled"
  (interactive)
...
(general-def
  ...
  "s" 'save-buffer
  "k" 'kill-this-buffer
  "i" 'hcps/indent-region-or-buffer
  "c" 'comment-dwim
