...
(use-package ssh
  :after shell
  :preface
  (defcustom ssh-server "bolsa.inf"
    "Indicates the remote server to be used when remotely using server with SSH (e.g. bolsa.inf)."
    :type 'string
    :safe #'stringp
    :group 'ssh)
  (defun ssh-connect-remote ()
    "Open a SSH session buffer to the desired target."
    (interactive)
    (ssh (read-from-minibuffer "What server to connect to: " nil nil nil ssh-server)))
  :custom
